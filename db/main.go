...
func initCategories(app core.App) error {
	...
	existingCategories := make(map[string]struct{}, len(records))
	for _, record := range records {
		existingCategories[util.NormalizeCategoryName(record.GetString("name"))] = struct{}{}
	}

	for _, element := range util.DefaultCategoryNames() {
		if _, ok := existingCategories[util.NormalizeCategoryName(element)]; !ok {
