<script lang="ts">
    ...
    /*function applySurfaceDataToGeoJSON(geo: FeatureCollection, surface?: TrailSurface) {
        if (!geo?.features?.length) {
            return;
        }

        for (const feature of geo.features ?? []) {
            const coordinateProperties = feature.properties?.coordinateProperties as Record<string, unknown> | undefined;
            if (coordinateProperties && "surfaces" in coordinateProperties) {
                delete coordinateProperties.surfaces;
            }
        }

        const surfacePoints = surface?.perPoint;
        ...
        type LineInfo = {
            ...
            commit: (values: (string | null)[] | undefined) => void;
        };
        ...
        for (const feature of geo.features ?? []) {
            ...
            const coordinateProperties = properties.coordinateProperties ?? (properties.coordinateProperties = {});

            if (geometry.type === "LineString") {
                const coords = geometry.coordinates as number[][];
                const surfaces = new Array(coords.length).fill(null);
                lines.push({
                    coordinates: coords,
                    surfaces,
                    commit: (values) => {
                        if (values && values.some((value) => value !== null)) {
                            coordinateProperties.surfaces = values;
                        } else if ("surfaces" in coordinateProperties) {
                            delete coordinateProperties.surfaces;
                        }
                    },
                });
            } else if (geometry.type === "MultiLineString") {
                const segments = geometry.coordinates as number[][][];
                const container: Array<(string | null)[] | undefined> = new Array(segments.length);
                segments.forEach((coords, idx) => {
                    ...
                    lines.push({
                        ...
                        surfaces,
                        commit: (values) => {
                            container[idx] = values;
                            const hasAnySurface = container.some(
                                (entry) => Array.isArray(entry) && entry.some((value) => value !== null)
                            );
                            if (hasAnySurface) {
                                coordinateProperties.surfaces = container;
                            } else if ("surfaces" in coordinateProperties) {
                                delete coordinateProperties.surfaces;
                            }
                        },
        ...
        const flattened: FlattenedCoordinate[] = [];
        let globalIndex = 0;
        ...
        const breakpoints = surfacePoints
            .map((point) => {
                if (!point?.type) {
                    return null;
                }
                ...
                for (const coord of flattened) {
                    const distance = distanceSquared(point.lat, point.lon, coord.lat, coord.lon);
                    if (distance < smallestDistance) {
                        smallestDistance = distance;
                        closestIndex = coord.globalIndex;
                    }
                }
        ...
        const dedupedBreakpoints: { index: number; type: string }[] = [];
        for (const bp of breakpoints) {
            const last = dedupedBreakpoints[dedupedBreakpoints.length - 1];
            if (last && last.index === bp.index) {
                last.type = bp.type;
            } else {
        ...
        for (const coord of flattened) {
            while (
                breakpointCursor < dedupedBreakpoints.length &&
                coord.globalIndex >= dedupedBreakpoints[breakpointCursor].index
            ) {
                currentType = dedupedBreakpoints[breakpointCursor].type;
                breakpointCursor += 1;
            }

            if (currentType !== undefined) {
                coord.line.surfaces[coord.indexInLine] = currentType;
            } else {
                coord.line.surfaces[coord.indexInLine] = null;
            }
        }

        for (const line of lines) {
            const normalized = line.surfaces.map((value) => (value ?? null));
            const hasAnySurface = normalized.some((value) => value !== null);
            line.commit(hasAnySurface ? normalized : undefined);
        }
    }*/
